...
def pretrain_one_epoch(model: torch.nn.Module,
    ...
    for data_iter_step, data in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        with amp_autocast():
            loss, _, _, total_aux_loss = model(samples, mask_ratio=args.mask_ratio)
...
def train_one_epoch(model: torch.nn.Module, criterion: torch.nn.Module,
    ...
    for data_iter_step, data in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        with amp_autocast():
            outputs, total_aux_loss = model(samples)
...
def evaluate(data_loader, model, device):
    ...
    for batch in metric_logger.log_every(data_loader, 10, header):
        ...
        with torch.cuda.amp.autocast():
            output, total_aux_loss = model(images)
    ...
    print(f"FPS (Samples per Second): {fps:.2f}")
